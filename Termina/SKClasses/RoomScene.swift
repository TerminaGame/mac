...
class RoomScene: SKScene {
    ...
    func setUpBackground(_ bg: Int?) {
        ...
        if bg == nil && self.name != "Tutorial" {
            ...
            if self.name == "20" {
                ...
                let randomLower = Int.random(in: 1...10)
    ...
    func setUpWeapon() {
        if childNode(withName: "weaponItemNode") != nil {
            ...
            if (gamePlayer?.level ?? 1) <= 31 {
                items.append(Weapon(name: NameGenerator().generateWeaponName(), equipLevel: Int.random(in: 1 ... 31), player: gamePlayer!, node: weaponNode!))
            } else {
                let chance = Int.random(in: 0...10)
                if (chance >= 3 && chance <= 6) {
                    items.append(Weapon(name: NameGenerator().generateWeaponName(), equipLevel: Int.random(in: 1...(gamePlayer?.level ?? 1) - 30), player: gamePlayer!, node: weaponNode!))
                } else {
